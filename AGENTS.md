...
- The bin entry `dist/cli.js` gates on the invoked file's realpath (`src/cli.ts` `isDirectEntry()`), so direct and symlinked bins (`npm link` / `npm install -g .`) both run `main()`. Keep the guard: tests import without side effects because `process.argv[1]` is undefined under vitest. Regression coverage: `test/entrypoint.test.ts` (spawns the built entry through a temporary symlink).
